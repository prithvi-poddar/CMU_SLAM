...
# -*- coding: utf-8 -*-
"""
Created on Sat Aug  1 00:02:12 2020
...
def bundle_adjustment(k, R, C, image1_pt, image2_pt, image1_3d):
    ...
    q_new = [qw+dqw, qx+dqx, qy+dqy, qz+dqz]
...
def check_reprojection_error(b, f_x, threshold):
    res = True
    for i in range(len(b)):
        if (abs(b[i][0]-f_x[i][0])>threshold):
            res = False
            break
    return res
...
def estimate_trajectory_BA(estimate_motion, matches, kp_list, k, depth_maps=[]):
    
    trajectory = [np.array([0, 0, 0])]
    orientation = [np.array([1, 0, 0, 0])]
    P = np.eye(4)
    R_3d = P[:3,:3]
    t_3d = P[:3,3].reshape(3,1)
    ...
    for a in range(100):
        ...
        C = ((-rmat.T).dot(t)).reshape([3,1])
        
        """ Preparing for bundle adjustment """ #remeber to add transformation
        image1_pt = []
        image2_pt = []
        for m in match:
        ...
        for i in range(len(image1_pt)):
            ...
            depth1 = depth[image1_y, image1_x]
            pt_3d = np.dot(np.linalg.inv(k), np.array([image1_x*depth1, image1_y*depth1, depth1]).reshape([3,1]))
            #pt_3d = t_3d + R_3d.dot(pt_3d)
        ...
        for i in range(20):
            if (check_reprojection_error(b, f_x, 5)):
                break
            
            else:
                R, C, f_x = bundle_adjustment(k, R, C, image1_pt, image2_pt, image1_3d)
                
        P_new = np.eye(4)
        P_new[0:3,0:3] = R
        P_new[0:3,3] = C.reshape(3)
        P = P.dot(P_new)
        R_3d = P[:3,:3]
        t_3d = P[:3,3].reshape(3,1)
        trajectory.append(P[:3,3])
        orientation.append(get_quaternions(P[0:3, 0:3]))
        print(a)
...
trajectory, orientation = estimate_trajectory_BA(estimate_motion, matches, kp_list, k, depth_maps=depth_maps)
